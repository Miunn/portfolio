...
export function PlaceholdersAndVanishInput({
  ...
  return (
    ...
    >
      <canvas
        className={cn(
          "absolute pointer-events-none  text-base transform scale-50 top-[20%] left-12 origin-top-left filter invert dark:invert-0 pr-20",
          !animating ? "opacity-0" : "opacity-100"
        )}
        ref={canvasRef}
      />

      <Search className="absolute left-4 top-1/2 z-50 -translate-y-1/2" />
      <input
        onChange={(e) => {
          if (!animating) {
